...
export default function RouteDetail({route, onClose, onPlaceClick, onEdit, places, isMobile}) {
  ...
  return <div style={{flex:1,minWidth:0,borderLeft:isMobile?"none":`1px solid ${C.border}`,display:"flex",flexDirection:"column",overflow:"hidden"}}>
    {/* Header */}
    <div style={{display:"flex",alignItems:"center",gap:14,padding:isMobile?"16px 20px":"14px 20px",borderBottom:`1px solid ${C.borderLight}`,flexShrink:0}}>
    ...
    <div style={{display:"flex",gap:10,padding:isMobile?"14px 20px":"12px 20px",borderTop:`1px solid ${C.borderLight}`,flexShrink:0}}>
      <button onClick={()=>onEdit?.(route)}
        style={{flex:1,padding:isMobile?12:10,borderRadius:16,fontSize:isMobile?15:13,fontWeight:500,border:"none",background:C.blueBg,color:C.blue,cursor:"pointer",fontFamily:"inherit"}}>
        Edit stops
      </button>
      <button onClick={()=>{if(route.route_url)window.open(route.route_url,"_blank");}}
        style={{flex:1,padding:isMobile?12:10,borderRadius:16,fontSize:isMobile?15:13,fontWeight:500,border:"none",background:C.blue,color:"#fff",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
